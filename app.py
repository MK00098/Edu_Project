...
# ─── 10) 상세 / 태그 목록 or 메인 리스트 ────────────────────────────────────────
if sel_title or sel_tag:
    # 홈 & 뒤로가기 버튼 양 끝 배치
    c1, _, c2 = st.columns([1, 20, 1])
    with c1:
        if st.button("🏠 홈"):
            clear_selection()
    with c2:
    ...
    if sel_title:
        ...
        with r1c1:
            st.button(kws[0].strip(), key=f"maj_{idx}_0")
        with r1c2:
            st.button(kws[1].strip(), key=f"maj_{idx}_1")
        # 둘째 줄: 3개
        r2c1, r2c2, r2c3 = st.columns([1,1,1])
        for i, col in enumerate([r2c1, r2c2, r2c3], start=2):
            with col:
                st.button(kws[i].strip(), key=f"maj_{idx}_{i}")

        # 교수 전략 & 추가 예시
        st.markdown("💡 **교수 전략**", unsafe_allow_html=True)
        st.info(row['교수 전략'])
        st.markdown("🧩 **추가 예시**", unsafe_allow_html=True)
        st.write(row['추가예시'])
